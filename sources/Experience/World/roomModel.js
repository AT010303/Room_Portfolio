...
export default class roomModel {
  constructor() {
    ...
    if (this.debug) {
      ...
      this.debugFolder
        .addBinding(this.colors, 'boardColor', { view: 'color' })
        .on('change', () => {
          this.model.material.uniforms.lightBoardColor.value.set(
            this.colors.boardColor
          );
        });
      ...
      this.debugFolder
        .addBinding(this.colors, 'pcColor', { view: 'color' })
        .on('change', () => {
          this.model.material.uniforms.lightPcColor.value.set(
            this.colors.pcColor
          );
        });
      ...
      this.debugFolder
        .addBinding(this.colors, 'deskColor', { view: 'color' })
        .on('change', () => {
          this.model.material.uniforms.lightDeskColor.value.set(
            this.colors.deskColor
          );
        });
  ...
  setmodel() {
    ...
    this.chairTvclockstuf = this.model.roomModel.children.find(
      (child) => child.name === 'chairTvclockstuf'
    );
    this.deskShelfStuf = this.model.roomModel.children.find(
      (child) => child.name === 'deskShelfStuf'
    );
    this.frame1 = this.model.roomModel.children.find(
      (child) => child.name === 'frame1'
    );
    this.frame2 = this.model.roomModel.children.find(
      (child) => child.name === 'frame2'
    );
    this.frame3 = this.model.roomModel.children.find(
      (child) => child.name === 'frame3'
    );
    this.laptop = this.model.roomModel.children.find(
      (child) => child.name === 'laptop'
    );
    this.monitor = this.model.roomModel.children.find(
      (child) => child.name === 'monitor'
    );
    this.roomFurniture = this.model.roomModel.children.find(
      (child) => child.name === 'roomFurniture'
    );
    this.smartphone = this.model.roomModel.children.find(
      (child) => child.name === 'smartphone'
    );
    this.tvdisplay = this.model.roomModel.children.find(
      (child) => child.name === 'tvdisplay'
    );
    this.plant = this.model.roomModel.children.find(
      (child) => child.name === 'plant'
    );
