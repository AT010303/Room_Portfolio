...
export default class RayCaster {
  constructor() {
    ...
    this.preLoader.on('start', () => {
      ...
      this.thitBoxMaterial = new THREE.MeshBasicMaterial({
        color: 0xff0000,
        wireframe: true
      });
      ...
      if (this.debug) {
        ...
        this.debugFolder
          .addBinding(this.hitboxVisibility, 'visible')
          .on('change', (ev) => {
      ...
      this.objectToTest = [this.roomview.monitor, this.roomview.laptop];

      //touch object
      if (this.config.touch == true) {
        this.objectToTest.push(this.desktopHitbox, this.laptopHitbox);
      } else {
        this.objectToTest.push(this.roomview.monitor, this.roomview.laptop);
      }
      ...
      window.addEventListener('pointerdown', (event) => {
        ...
        this.cursorXmin = Math.abs(
          ((event.clientX / this.size.width) * 2 - 1) * 0.9
        );
        this.cursorXmax = Math.abs(
          ((event.clientX / this.size.width) * 2 - 1) * 1.1
        );

        this.cursorYmin = Math.abs(
          ((event.clientY / this.size.height) * 2 - 1) * 0.9
        );
        this.cursorYmax = Math.abs(
          ((event.clientY / this.size.height) * 2 - 1) * 1.1
        );
      });
